...
export default async function autoDownloadModel(
	...
	if (!autoDownloadModelName) {
		throw new Error('[Whisper-node] Error: Model name must be provided.')
	}

	if (!MODELS_LIST.includes(autoDownloadModelName)) {
		throw new Error('[Whisper-node] Error: Provide a valid model name')
	}
	...
	try {
		...
		if (modelAlreadyExist) {
			logger.debug(`[Whisper-node] ${autoDownloadModelName} already exist. Skipping download.`)
			return 'Models already exist. Skipping download.'
		}

		logger.debug(`[Whisper-node] Auto-download Model: ${autoDownloadModelName}`)
		...
		if (result.code !== 0) {
			throw new Error(`[Whisper-node] Failed to download model: ${result.stderr}`)
		}

		logger.debug('[Whisper-node] Model downloaded. Attempting to build whisper.cpp...')
		shell.cd('../')
		...
		logger.error('[Whisper-node] Error caught in autoDownloadModel:', error)
		shell.cd(projectDir)
		throw error
	}
}
