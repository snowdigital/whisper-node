...
export const constructCommand = (filePath: string, args: IOptions): string => {
	...
	if (!args.modelName) {
		errors.push('[Whisper-node] Error: Provide model name')
	}

	if (!MODELS_LIST.includes(args.modelName)) {
		errors.push(`[Whisper-node] Error: Enter a valid model name. Available models are: ${MODELS_LIST.join(', ')}`)
	}

	const modelPath = path.join(WHISPER_CPP_PATH, 'models', MODEL_OBJECT[args.modelName])
	if (!fs.existsSync(modelPath)) {
		errors.push(
			'[Whisper-node] Error: Model file does not exist. Please ensure the model is downloaded and correctly placed.'
		)
	}
	...
	if (!executablePath) {
		throw new Error('[Whisper-node] Error: whisper-cli executable not found')
	}
...
const constructOptionsFlags = (args: IOptions): string => {
	const flags = [
		args.whisperOptions?.noTimestamps ? '--no-timestamps ' : '',
		args.whisperOptions?.noPrints ? '--no-prints ' : '',
		args.whisperOptions?.printColors ? '--print-colors ' : '',
