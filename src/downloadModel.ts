...
import { MODELS_LIST, DEFAULT_MODEL, WHISPER_CPP_PATH, MODEL_OBJECT } from './constants'
import fs from 'fs'
import { Logger } from './types'
const askForModel = async (logger: Logger = console): Promise<string> => {
	const answer = await readlineSync.question(
		`\n[Whisper-node] Enter model name (e.g. 'tiny.en') or 'cancel' to exit\n(ENTER for tiny.en): `
	)

	if (answer === 'cancel') {
		logger.log('[Whisper-node] Exiting model downloader.\n')
		process.exit(0)
	}
	// User presses enter
	else if (answer === '') {
		logger.log('[Whisper-node] Going with', DEFAULT_MODEL)
...
const askIfUserWantToUseCuda = async (logger: Logger = console) => {
	const answer = await readlineSync.question(
		`\n[Whisper-node] Do you want to use CUDA for compilation? (y/n)\n(ENTER for n): `
	)

	if (answer === 'y') {
		logger.log('[Whisper-node] Using CUDA for compilation.')
		return true
	} else {
		logger.log('[Whisper-node] Not using CUDA for compilation.')
		return false
	}
}
...
async function downloadModel(logger: Logger = console) {
	try {
		...
		if (anyModelExist.length > 0) {
			logger.log('\n[Whisper-node] Currently installed models:')
			anyModelExist.forEach(model => logger.log(`- ${model}`))
			logger.log('\n[Whisper-node] You can install additional models from the list below.\n')
		}
...
`)
		...
		if (!shell.which('./download-ggml-model.sh')) {
			throw '[Whisper-node] Error: Downloader not found.\n'
		}
		...
		logger.log('[Whisper-node] Attempting to build whisper.cpp...\n')
		shell.cd('../')

		const withCuda = await askIfUserWantToUseCuda()

		// Use CMake instead of make
		logger.log('[Whisper-node] Configuring CMake build...')
		...
		logger.log('[Whisper-node] Building with CMake...')
		shell.exec('cmake --build build --config Release')

		process.exit(0)
	} catch (error) {
		logger.error('[Whisper-node] Error Caught in downloadModel\n')
		logger.error(error)
		return error
	}
}
